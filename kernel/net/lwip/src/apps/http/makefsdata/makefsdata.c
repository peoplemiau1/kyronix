...
/* OUT_BUF_SIZE is the size of the output buffer used during decompression.
   OUT_BUF_SIZE must be a power of 2 >= TINFL_LZ_DICT_SIZE (because the low-level decompressor not
   only writes, but reads from the output buffer as it decompresses) */
#define OUT_BUF_SIZE COPY_BUFSIZE
static uint8 s_outbuf[OUT_BUF_SIZE];
static uint8 s_checkbuf[OUT_BUF_SIZE];

#ifndef MAKEFS_SUPPORT_DEFLATE_ZLIB
/* tdefl_compressor contains all the state needed by the low-level compressor so it's a pretty big
   struct (~300k). This example makes it a global vs. putting it on the stack, of course in
   real-world usage you'll probably malloc() or new it. */
...
int file_write_http_header(FILE *data_file, const char *filename, int file_size,
                           u16_t *http_hdr_len, u16_t *http_hdr_chksum, u8_t provide_content_len,
                           int is_compressed);
...
static void print_usage(void) {
    printf(" Usage: htmlgen [targetdir] [-s] [-e] [-11] [-nossi] [-ssi:<filename>] [-c] "
           "[-f:<filename>] [-m] [-svr:<name>] [-x:<ext_list>] [-xc:<ext_list>" USAGE_ARG_DEFLATE
               NEWLINE NEWLINE);
    printf("   targetdir: relative or absolute path to files to convert" NEWLINE);
    printf("   switch -s: toggle processing of subdirectories (default is on)" NEWLINE);
    printf("   switch -e: exclude HTTP header from file (header is created at runtime, default is "
           "off)" NEWLINE);
    printf("   switch -11: include HTTP 1.1 header (1.0 is default)" NEWLINE);
    printf(
        "   switch -nossi: no support for SSI (cannot calculate Content-Length for SSI)" NEWLINE);
    printf("   switch -ssi: ssi filename (ssi support controlled by file list, not by "
           "extension)" NEWLINE);
    ...
    printf("   switch -x: comma separated list of extensions of files to exclude (e.g., "
           "-x:json,txt)" NEWLINE);
    printf("   switch -xc: comma separated list of extensions of files to not compress (e.g., "
           "-xc:mp3,jpg)" NEWLINE);
#if MAKEFS_SUPPORT_DEFLATE
    printf("   switch -defl: deflate-compress all non-SSI files (with opt. compr.-level, "
           "default=10)" NEWLINE);
    printf("                 ATTENTION: browser has to support \"Content-Encoding: "
           "deflate\"!" NEWLINE);
#endif
...
int main(int argc, char *argv[]) {
     ...
     */
    fprintf(data_file, "#ifndef FS_FILE_FLAGS_HEADER_INCLUDED" NEWLINE
                       "#define FS_FILE_FLAGS_HEADER_INCLUDED 1" NEWLINE "#endif" NEWLINE);
    /* define FS_FILE_FLAGS_HEADER_PERSISTENT to 0 if not defined (compatibility with older
     * httpd/fs: wasn't supported back then) */
    fprintf(data_file, "#ifndef FS_FILE_FLAGS_HEADER_PERSISTENT" NEWLINE
...
#endif
    fprintf(data_file,
            "#ifndef FSDATA_ALIGN_PRE" NEWLINE "#define FSDATA_ALIGN_PRE" NEWLINE "#endif" NEWLINE);
    fprintf(data_file, "#ifndef FSDATA_ALIGN_POST" NEWLINE "#define FSDATA_ALIGN_POST" NEWLINE
                       "#endif" NEWLINE);
#if ALIGN_PAYLOAD
    fprintf(data_file, "#if FSDATA_FILE_ALIGNMENT==2" NEWLINE
    ...
    if (remove("fsdata.tmp") != 0) { printf("Warning: failed to delete fsdata.tmp\n"); }
    ...
    if (ssi_file_buffer) { free(ssi_file_buffer); }
    if (ssi_file_lines) { free(ssi_file_lines); }
...
static void copy_file(const char *filename_in, FILE *fout) {
    ...
    while ((len = fread(buf, 1, COPY_BUFSIZE, fin)) > 0) { fwrite(buf, 1, len, fout); }
    free(buf);
    fclose(fin);
}
            ...
            for (i = 0; i < dir.n_files; i++) {
                ...
                if (ret == 0) {
                    ...
                    if (freelen > 0) {
                        ...
                        printf("WARNING: cannot process sub due to path length restrictions: "
                               "\"%s/%s\"\n",
                               curSubdir, currName);
                    }
                }
            ...
            for (i = 0; i < dir.n_files; i++) {
                ...
                if (ret == 0) {
                    if (!file.is_dir) {
                        ...
                        if (strcmp(curName, "fsdata.tmp") == 0) { continue; }
                        if (strcmp(curName, "fshdr.tmp") == 0) { continue; }
...
static u8_t *get_file_data(const char *filename, int *file_size, int can_be_compressed,
                           int *is_compressed) {
    FILE *inFile;
    size_t fsize = 0;
    u8_t *buf;
            ...
            if (fsize < OUT_BUF_SIZE) {
...
#ifndef MAKEFS_SUPPORT_DEFLATE_ZLIB
                /* create tdefl() compatible flags (we have to compose the low-level flags
                 * ourselves, or use tdefl_create_comp_flags_from_zip_params() but that means
                 * MINIZ_NO_ZLIB_APIS can't be defined). */
                mz_uint comp_flags = s_tdefl_num_probes[MZ_MIN(10, deflate_level)] |
                                     ((deflate_level <= 3) ? TDEFL_GREEDY_PARSING_FLAG : 0);
                if (!deflate_level) { comp_flags |= TDEFL_FORCE_ALL_RAW_BLOCKS; }
                ...
                if (out_bytes < fsize) {
                    ...
                    memcpy(ret_buf, s_outbuf, out_bytes);
                    {
                        ...
                        tinfl_init(&inflator);
                        dec_status =
                            tinfl_decompress(&inflator, (const mz_uint8 *) ret_buf, &dec_in_bytes,
                                             s_checkbuf, (mz_uint8 *) next_out, &dec_out_bytes, 0);
                    ...
                    buf = ret_buf;
                    *file_size = out_bytes;
                    printf(" - deflate: %d bytes -> %d bytes (%.02f%%)" NEWLINE, (int) fsize,
                           (int) out_bytes, (float) ((out_bytes * 100.0) / fsize));
...
static int write_checksums(FILE *struct_file, const char *varname, u16_t hdr_len, u16_t hdr_chksum,
...
static void fix_filename_for_c(char *qualifiedName, size_t max_len) {
    ...
    for (i = 0; i < len; i++) {
        if (!is_valid_char_for_c_var(new_name[i])) { new_name[i] = '_'; }
    }
    do {
        filename_ok = 1;
    ...
    if (supportSsi) {
        if (ssi_file_buffer) {
            ...
            for (i = 0; i < ssi_file_num_lines; i++) {
                ...
                if (!strcmp(&curSubdir[1], listed_file)) { ret = 1; }
            }
            curSubdir[sublen] = 0;
            return ret;
            ...
            for (loop = 0; loop < NUM_SHTML_EXTENSIONS; loop++) {
                if (strstr(filename, g_pcSSIExtensions[loop])) { return 1; }
            }
...
static int ext_in_list(const char *filename, const char *ext_list) {
    ...
    while (*ext != '\0') {
        ...
        if (comma == NULL) { comma = strchr(ext, '\0'); }
        ext_size = comma - ext;
...
int process_file(FILE *data_file, FILE *struct_file, const char *filename) {
...
#endif /* ALIGN_PAYLOAD */
    fprintf(data_file,
            "static const unsigned char FSDATA_ALIGN_PRE data_%s[] FSDATA_ALIGN_POST = {" NEWLINE,
            varname);
    ...
    if (is_ssi) { flags |= FS_FILE_FLAGS_SSI; }
    ...
    if (includeHttpHeader) {
        file_write_http_header(data_file, filename, file_size, &http_hdr_len, &http_hdr_chksum,
                               has_content_len, is_compressed);
        flags |= FS_FILE_FLAGS_HEADER_INCLUDED;
        if (has_content_len) {
            flags |= FS_FILE_FLAGS_HEADER_PERSISTENT;
            if (useHttp11) { flags |= FS_FILE_FLAGS_HEADER_HTTPVER_1_1; }
        }
    }
    if (precalcChksum) {
        chksum_count = write_checksums(struct_file, varname, http_hdr_len, http_hdr_chksum,
                                       file_data, file_size);
    }
    ...
    if (flags & FS_FILE_FLAGS_HEADER_PERSISTENT) {
        if (flags_printed) { fputs(" | ", struct_file); }
    ...
    if (!flags_printed) { fputs("0", struct_file); }
...
int file_write_http_header(FILE *data_file, const char *filename, int file_size,
    ...
    if (provide_content_len) {
        ...
        cur_len = strlen(cur_string);
        fprintf(data_file, NEWLINE "/* \"%s%d\r\n\" (%" SZT_F "+ bytes) */" NEWLINE, cur_string,
                content_len, cur_len + 2);
        written += file_put_ascii(data_file, cur_string, cur_len, &i);
        if (precalcChksum) {
            memcpy(&hdr_buf[hdr_len], cur_string, cur_len);
            hdr_len += cur_len;
...
int file_put_ascii(FILE *file, const char *ascii_string, size_t len, int *i) {
    ...
    for (x = 0; x < len; x++) {
        ...
        if ((++(*i) % HEX_BYTES_PER_LINE) == 0) { fprintf(file, NEWLINE); }
    }
    return (int) len;
}
