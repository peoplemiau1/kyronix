...
static snmp_err_t usmusertable_get_instance(const u32_t *column, const u32_t *row_oid,
                                            u8_t row_oid_len,
    ...
    if (!snmp_oid_in_range(&row_oid[engineid_start], engineid_len, usmUserTable_oid_ranges,
                           engineid_len)) {
        return SNMP_ERR_NOSUCHINSTANCE;
    }
...
static snmp_err_t usmusertable_get_next_instance(const u32_t *column, struct snmp_obj_id *row_oid,
    ...
    if (row_oid->len > 0) {
        ...
        if (engineid_len > row_oid->len) {
            ...
            if (!snmp_oid_equal(&row_oid->id[engineid_start], row_oid->len - 1, engineid_oid,
                                row_oid->len - 1)) {
                return SNMP_ERR_NOSUCHINSTANCE;
            }
        } else {
            ...
            snmp_engineid_to_oid(engineid, engineid_oid, engineid_len);
            if (!snmp_oid_equal(&row_oid->id[engineid_start], engineid_len, engineid_oid,
                                engineid_len)) {
                return SNMP_ERR_NOSUCHINSTANCE;
            }
        }
        ...
        if (row_oid->len > engineid_start + engineid_len) {
            ...
            if (row_oid->len < engineid_len + name_len + 2) {
                ...
                if (!snmp_oid_in_range(&row_oid->id[name_start], tmplen, usmUserTable_oid_ranges,
                                       tmplen)) {
                    return SNMP_ERR_NOSUCHINSTANCE;
                }
            } else {
                ...
                u8_t tmplen = row_oid->len - engineid_len - 2;
                if (!snmp_oid_in_range(&row_oid->id[name_start], name_len, usmUserTable_oid_ranges,
                                       tmplen)) {
                    return SNMP_ERR_NOSUCHINSTANCE;
                }
            }

            /* At this point the EngineID and (partial) UserName match the local EngineID and
             * UserName.*/
        }
    }
    ...
    for (i = 0; i < snmpv3_get_amount_of_users(); i++) {
        ...
        snmp_next_oid_check(&state, test_oid, (u8_t) (1 + eid_len + 1 + strlen(username)),
...
static s16_t usmusertable_get_value(struct snmp_node_instance *cell_instance, void *value) {
    ...
    case 5: { /* usmUserAuthProtocol */
        ...
        snmpv3_get_user((const char *) cell_instance->reference.ptr, &auth_algo_val, NULL, NULL,
                        NULL);
    ...
    case 8: { /* usmUserPrivProtocol */
        ...
        snmpv3_get_user((const char *) cell_instance->reference.ptr, NULL, NULL, &priv_algo_val,
                        NULL);
...
static s16_t usmstats_scalars_get_value(const struct snmp_scalar_array_node_def *node,
                                        void *value) {
...
static const struct snmp_table_node usmusertable =
    SNMP_TABLE_CREATE(2, usmusertable_columns, usmusertable_get_instance,
...
static const struct snmp_scalar_array_node usmstats_scalars = SNMP_SCALAR_CREATE_ARRAY_NODE(
    1, usmstats_scalars_nodes, usmstats_scalars_get_value, NULL, NULL);

static const struct snmp_node *const usmmibobjects_subnodes[] = { &usmstats_scalars.node.node,
...
const struct snmp_mib snmpusmmib = { snmpusmmib_base_oid, LWIP_ARRAYSIZE(snmpusmmib_base_oid),
