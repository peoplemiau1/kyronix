...
static bool pncrypt_setkey(int timeoffs) {
    ...
    SHA1Final(dig, &ctxt);
    /* FIXME: if we want to do SRP, we need to find a way to pass the PolarSSL des_context instead
     * of using static memory */
...
static void eap_lowerdown(ppp_pcb *pcb) {
    ...
    if (eap_server_active(pcb)) {
        if (pcb->settings.eap_timeout_time > 0) { UNTIMEOUT(eap_server_timeout, pcb); }
    } else {
...
static void eap_send_response(ppp_pcb *pcb, u_char id, u_char typenum, const u_char *str,
    ...
    if (lenstr > 0) { MEMCPY(outp, str, lenstr); }
...
static void eap_chap_response(ppp_pcb *pcb, u_char id, u_char *hash, const char *name,
    ...
    if (namelen > 0) { MEMCPY(outp, name, namelen); }
...
static void eap_request(ppp_pcb *pcb, u_char *inp, int id, int len) {
     ...
     */
    if (pcb->eap.es_client.ea_state <= eapClosed) return;
    ...
    if (pcb->settings.eap_allow_req != 0 &&
        ...
        if (pcb->settings.eap_req_time > 0) { UNTIMEOUT(eap_client_timeout, pcb); }
        auth_withpeer_fail(pcb, PPP_EAP);
        return;
    }
    ...
    case EAPT_SRP:
        ...
        case EAPSRP_CHALLENGE:
            ...
            if (tc == NULL) {
                ...
                if (rhostnamelen > MAXNAMELEN) { rhostnamelen = MAXNAMELEN; }
...
client_failure:
    pcb->eap.es_client.ea_state = eapBadAuth;
    if (pcb->settings.eap_req_time > 0) { UNTIMEOUT(eap_client_timeout, (void *) esp); }
    pcb->eap.es_client.ea_session = NULL;
    t_clientclose(tc);
    auth_withpeer_fail(pcb, PPP_EAP);
#endif /* USE_SRP */
...
static void eap_response(ppp_pcb *pcb, u_char *inp, int id, int len) {
     ...
     */
    if (pcb->eap.es_server.ea_state <= eapClosed) return;
    ...
    case EAP_REQUEST:
        ...
        case EAPT_SRP:
            ...
            case EAPSRP_SVALIDATOR:
                ...
                if (uval != 0) { printer(arg, " f<%X>", uval); }
