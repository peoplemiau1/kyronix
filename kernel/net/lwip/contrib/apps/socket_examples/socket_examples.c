...
static void sockex_select_waiter(void *arg) {
    ...
    FD_ZERO(&writeset);
    FD_ZERO(&errset);
    if (helper->wait_read) { FD_SET(helper->socket, &readset); }
    if (helper->wait_write) { FD_SET(helper->socket, &writeset); }
    if (helper->wait_err) { FD_SET(helper->socket, &errset); }
...
static void sockex_testtwoselects(void *arg) {
    int s1;
    int s2;
    int ret;
#if LWIP_IPV6
...
#else
static void sockex_testtwoselects(void *arg) { LWIP_UNUSED_ARG(arg); }
#endif

#if !SOCKET_EXAMPLES_RUN_PARALLEL
static void socket_example_test(void *arg) {
    sys_msleep(1000);
