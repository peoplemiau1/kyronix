...
void sio_expect_string(u8_t *str, sio_status_t *siostat) {
    ...
    while (1) {
        ...
        if (0 == str[finger]) break; /* done, we have a match */
    }
...
sio_fd_t sio_open(u8_t devnum) {
    ...
    if ((devnum == 1) || (devnum == 0)) {
        ...
        if (childpid == 0) {
            execl("/usr/sbin/pppd", "pppd", "ms-dns", "198.168.100.7", "local", "crtscts", "debug",
#ifdef LWIP_PPP_CHAP_TEST
                  "auth", "require-chap", "remotename", "lwip",
#else
                  "noauth",
#endif
#if LWIP_IPV6
        ...
        if (childpid == 0) {
            ...
            execl("/sbin/slattach", "slattach", "-d", "-v", "-L", "-p", "slip",
                  ptsname(siostate->fd), NULL);
            ...
            LWIP_DEBUGF(SIO_DEBUG, ("sio_open[%d]: spawned slattach pid %d on %s\n", siostate->fd,
                                    childpid, ptsname(siostate->fd)));
            /* wait a moment for slattach startup */
            sleep(1);
            /* configure SLIP interface on host side as P2P interface */
            snprintf(buf, sizeof(buf), "/sbin/ifconfig sl0 mtu %d %s pointopoint %s up",
...
void sio_change_baud(sioBaudrates baud, sio_status_t *siostat) {
    ...
    default:
        LWIP_DEBUGF(SIO_DEBUG,
                    ("sio_change_baud[%d]: Unknown baudrate, code:%d\n", siostat->fd, baud));
        break;
    }
}
