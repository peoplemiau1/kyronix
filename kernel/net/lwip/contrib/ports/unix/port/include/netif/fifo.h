...
typedef struct fifo_t {
    ...
    int len; /* len probably not needed, may be calculated from dataslot and emptyslot in
                conjunction with FIFOSIZE */
    ...
    u8_t getWaiting;  /* flag used to indicate that fifoget is waiting for data. fifoput is supposed
                         to clear */
...
/**
 *	fifoput is called by the signalhandler when new data has arrived (or some other event is
 * indicated) fifoput reads directly from the serialport and is thus highly dependent on unix arch
 * at this moment
 *	@param 	fifo pointer to fifo data structure
