...
struct pcapifh_linkstate *pcapifh_linkstate_init(char *adapter_name) {
    struct pcapifh_linkstate *state =
        (struct pcapifh_linkstate *) malloc(sizeof(struct pcapifh_linkstate));
    if (state != NULL) {
        memset(state, 0, sizeof(struct pcapifh_linkstate));
        state->ppacket_oid_data =
...
enum pcapifh_link_event pcapifh_linkstate_get(struct pcapifh_linkstate *state) {
    ...
    if (state != NULL) {
        ...
        if (PacketRequest(state->lpAdapter, FALSE, state->ppacket_oid_data)) {
            ...
            ret = ((fNdisMediaState == NdisMediaStateConnected) ? PCAPIF_LINKEVENT_UP :
...
void pcapifh_linkstate_close(struct pcapifh_linkstate *state) {
    if (state != NULL) {
        if (state->lpAdapter != NULL) { PacketCloseAdapter(state->lpAdapter); }
        if (state->ppacket_oid_data != NULL) { free(state->ppacket_oid_data); }
        free(state);
    }
}
...
void pcapifh_linkstate_close(struct pcapifh_linkstate *state) { LWIP_UNUSED_ARG(state); }
