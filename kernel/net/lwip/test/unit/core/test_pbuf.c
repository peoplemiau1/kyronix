...
static void pbuf_setup(void) { lwip_check_ensure_no_alloc(SKIP_POOL(MEMP_SYS_TIMEOUT)); }

static void pbuf_teardown(void) { lwip_check_ensure_no_alloc(SKIP_POOL(MEMP_SYS_TIMEOUT)); }
...
START_TEST(test_pbuf_copy_unmatched_chains) {
    ...
    for (i = 0; i < 8; i++) {
        ...
        fail_unless(p != NULL);
        for (j = 0; j < p->len; j++) { ((u8_t *) p->payload)[j] = (u8_t) ((i << 4) | j); }
        if (source) {
            pbuf_cat(source, p);
        } else {
            source = p;
    ...
    for (i = 0; i < source->tot_len; i++) { fail_unless(pbuf_get_at(source, i) == i); }
    ...
    for (i = 0; i < source->tot_len; i++) { fail_unless(pbuf_get_at(dest, i) == i); }
...
START_TEST(test_pbuf_queueing_bigger_than_64k) {
    ...
    for (i = 0; i < TESTBUFSIZE_1; i++) { testbuf_1[i] = (u8_t) rand(); }
    for (i = 0; i < TESTBUFSIZE_2; i++) { testbuf_2[i] = (u8_t) rand(); }
    for (i = 0; i < TESTBUFSIZE_3; i++) { testbuf_3[i] = (u8_t) rand(); }
...
START_TEST(test_pbuf_take_at_edge) {
    ...
    for (i = 0; i < (int) sizeof(testdata); i++) {
        fail_unless(out[i] == testdata[i], "Bad data at pos %d, was %02X, expected %02X", i, out[i],
                    testdata[i]);
    }
    ...
    out = (u8_t *) p->payload;
    fail_unless(out[p->len - 1] == testdata[0], "Bad data at pos %d, was %02X, expected %02X",
                p->len - 1, out[p->len - 1], testdata[0]);
    out = (u8_t *) q->payload;
    for (i = 1; i < (int) sizeof(testdata); i++) {
        fail_unless(out[i - 1] == testdata[i], "Bad data at pos %d, was %02X, expected %02X",
                    p->len - 1 + i, out[i - 1], testdata[i]);
    }
    ...
    out = (u8_t *) p->payload;
    for (i = 0; i < (int) sizeof(testdata); i++) {
        fail_unless(out[i] == testdata[i], "Bad data at pos %d, was %02X, expected %02X",
                    p->len + i, out[i], testdata[i]);
    }
    pbuf_free(p);
}
END_TEST
...
START_TEST(test_pbuf_get_put_at_edge) {
    ...
    out = (u8_t *) q->payload;
    fail_unless(*out == testdata, "Bad data at pos %d, was %02X, expected %02X", p->len, *out,
                testdata);

    getdata = pbuf_get_at(p, p->len);
    fail_unless(*out == getdata,
                "pbuf_get_at() returned bad data at pos %d, was %02X, expected %02X", p->len,
                getdata, *out);
    pbuf_free(p);
}
END_TEST
