...
static void netif_setup(void) { lwip_check_ensure_no_alloc(SKIP_POOL(MEMP_SYS_TIMEOUT)); }

static void netif_teardown(void) { lwip_check_ensure_no_alloc(SKIP_POOL(MEMP_SYS_TIMEOUT)); }
...
static void test_netif_ext_callback_dummy(struct netif *netif, netif_nsc_reason_t reason,
...
static void test_netif_ext_callback(struct netif *netif, netif_nsc_reason_t reason,
                                    const netif_ext_callback_args_t *args) {
...
START_TEST(test_netif_extcallbacks) {
    ...
    IP4_ADDR(&gw, 1, 2, 3, 254);
    expected_reasons =
        (netif_nsc_reason_t) (LWIP_NSC_IPV4_ADDRESS_CHANGED | LWIP_NSC_IPV4_NETMASK_CHANGED |
    ...
    expected_reasons =
        (netif_nsc_reason_t) (LWIP_NSC_IPV4_ADDRESS_CHANGED | LWIP_NSC_IPV4_SETTINGS_CHANGED |
                              LWIP_NSC_IPV4_ADDR_VALID);
    callback_ctr = 0;
    netif_set_addr(&net_test, &addr, &netmask, &gw);
    fail_unless(callback_ctr == 1);
...
Suite *netif_suite(void) {
    testfunc tests[] = { TESTFUNC(test_netif_extcallbacks), TESTFUNC(test_netif_flag_set),
                         TESTFUNC(test_netif_find) };
    return create_suite("NETIF", tests, sizeof(tests) / sizeof(testfunc), netif_setup,
                        netif_teardown);
}
