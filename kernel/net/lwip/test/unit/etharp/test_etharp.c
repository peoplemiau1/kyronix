...
static void default_netif_add(void) {
    ...
    netif_set_default(netif_add(&test_netif, &test_ipaddr, &test_netmask, &test_gw, NULL,
...
static void create_arp_response(ip4_addr_t *adr) {
    ...
    struct pbuf *p =
        pbuf_alloc(PBUF_RAW, sizeof(struct eth_hdr) + sizeof(struct etharp_hdr), PBUF_RAM);
    if (p == NULL) { FAIL_RET(); }
...
START_TEST(test_etharp_table) {
    ...
    if (netif_default != &test_netif) { fail("This test needs a default netif"); }
    ...
    if (pcb != NULL) {
        ...
        int i;
        for (i = 0; i < ARP_TABLE_SIZE + 2; i++) { IP4_ADDR(&adrs[i], 192, 168, 0, i + 2); }
...
Suite *etharp_suite(void) {
    ...
    return create_suite("ETHARP", tests, sizeof(tests) / sizeof(testfunc), etharp_setup,
                        etharp_teardown);
}
