...
static void test_netif_add(void) {
    ...
    netif_add(&test_netif, &test_ipaddr, &test_netmask, &test_gw, NULL, test_netif_init, NULL);
    netif_set_default(&test_netif);
    netif_set_up(&test_netif);
}

static void test_netif_remove(void) {
    if (netif_default == &test_netif) { netif_remove(&test_netif); }
}
...
static void ip4_setup(void) { lwip_check_ensure_no_alloc(SKIP_POOL(MEMP_SYS_TIMEOUT)); }
...
START_TEST(test_ip4_frag) {
    ...
    err =
        ip4_output_if_src(data, &test_ipaddr, ip_2_ip4(&peer_ip), 16, 0, IP_PROTO_UDP, &test_netif);
    fail_unless(err == ERR_OK);
...
START_TEST(test_ip4_icmp_replylen_short) {
    ...
    const u8_t unknown_proto[] = { 0x45, 0x00, 0x00, 0x15, 0xd4, 0x31, 0x00, 0x00, 0xff, 0x22, 0x66,
                                   0x41, 0xc0, 0xa8, 0x00, 0x02, 0xc0, 0xa8, 0x00, 0x01, 0xaa };
    struct pbuf *p;
...
START_TEST(test_ip4_icmp_replylen_first_8) {
    ...
    const u8_t unknown_proto[] = { 0x45, 0x00, 0x00, 0x1f, 0xd4, 0x31, 0x00, 0x00, 0xff, 0x22, 0x66,
                                   0x37, 0xc0, 0xa8, 0x00, 0x02, 0xc0, 0xa8, 0x00, 0x01, 0xa0, 0xa1,
