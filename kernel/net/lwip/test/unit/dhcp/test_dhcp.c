...
static u8_t dhcp_offer[] = {
    ...
    0x01,
    0x06, /* Hw Ethernet, 6 bytes addrlen */
    0x00, /* 0 hops */
    0xAA,
    0xAA,
    0xAA,
    0xAA, /* Transaction id, will be overwritten */
    0x00,
    0x00, /* 0 seconds elapsed */
    0x00,
    0x00, /* Flags (unicast) */
    0x00,
    0x00,
    ...
    0x35,
    0x01,
    0x02, /* Message type: Offer */
    0x36,
    0x04,
    0xc3,
    0xaa,
    0xbd,
    0xab, /* Server identifier (IP) */
    0x33,
    0x04,
    0x00,
    0x00,
    0x00,
    0x78, /* Lease time 2 minutes */
    0x03,
    0x04,
    0xc3,
    0xaa,
...
static u8_t dhcp_ack[] = {
    ...
    0x35,
    0x01,
    0x05, /* Dhcp message type ack */
    0x36,
    0x04,
    0xc3,
    0xaa,
    0xbd,
    0xab, /* DHCP server identifier */
    0x33,
    0x04,
    0x00,
    0x00,
...
static void tick_lwip(void) {
    ...
    acd_tmr();
#endif
    if (tick % 5 == 0) { dhcp_fine_tmr(); }
    if (tick % 600 == 0) { dhcp_coarse_tmr(); }
}

static u32_t get_bad_xid(const struct netif *netif) { return ~netif_dhcp_data(netif)->xid; }
...
static void check_pkt(struct pbuf *p, u32_t pos, const u8_t *mem, u32_t len) {
    ...
    data = (u8_t *) p->payload;
    fail_if(memcmp(&data[pos], mem, len), "data at pos %d, len %d in packet %d did not match", pos,
            len, txpacket);
}
...
static err_t lwip_tx_func(struct netif *netif, struct pbuf *p) {
    ...
    if (debug) {
        ...
        do {
            int i;
            for (i = 0; i < pp->len; i++) { printf(" %02X", ((u8_t *) pp->payload)[i]); }
    ...
    case TEST_LWIP_DHCP:
        ...
        case 2: {
            ...
            const u8_t bootp_start[] = { 0x01, 0x01, 0x06,
                                         0x00 }; /* bootp request, eth, hwaddr len 6, 0 hops */
            ...
            if (txpacket == 1) {
                ...
                u8_t requested_ipaddr[] = {
                    0x32, 0x04, 0xc3, 0xaa, 0xbd, 0xc8
                }; /* Ask for offered IP */
...
START_TEST(test_dhcp) {
    ...
    for (i = 0; i < 200; i++) { tick_lwip(); }
    fail_unless(txpacket == (2 + DHCP_TEST_NUM_ARP_FRAMES), "TX %d packets, expected %d", txpacket,
                (2 + DHCP_TEST_NUM_ARP_FRAMES));
...
START_TEST(test_dhcp_relayed) {
    u8_t relay_offer[] = {
        0x00, 0x23, 0xc1, 0xde, 0xd0, 0x0d, 0x00, 0x22, 0x93, 0x5a, 0xf7, 0x60, 0x08, 0x00, 0x45,
        0x00, 0x01, 0x38, 0xfd, 0x53, 0x00, 0x00, 0x40, 0x11, 0x78, 0x46, 0x4f, 0x8a, 0x32, 0x02,
        0x4f, 0x8a, 0x33, 0x05, 0x00, 0x43, 0x00, 0x44, 0x01, 0x24, 0x00, 0x00, 0x02, 0x01, 0x06,
        0x00, 0x51, 0x35, 0xb6, 0xa0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4f, 0x8a,
        0x33, 0x05, 0x00, 0x00, 0x00, 0x00, 0x0a, 0xb5, 0x04, 0x01, 0x00, 0x23, 0xc1, 0xde, 0xd0,
        ...
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x63, 0x82, 0x53, 0x63, 0x01, 0x04, 0xff,
        0xff, 0xfe, 0x00, 0x03, 0x04, 0x4f, 0x8a, 0x32, 0x01, 0x06, 0x08, 0x4f, 0x8a, 0x00, 0xb4,
        0x55, 0x08, 0x1f, 0xd1, 0x1c, 0x04, 0x4f, 0x8a, 0x33, 0xff, 0x33, 0x04, 0x00, 0x00, 0x54,
        0x49, 0x35, 0x01, 0x02, 0x36, 0x04, 0x0a, 0xb5, 0x04, 0x01, 0xff
    };

    u8_t relay_ack1[] = {
        0x00, 0x23, 0xc1, 0xde, 0xd0, 0x0d, 0x00, 0x22, 0x93, 0x5a, 0xf7, 0x60, 0x08, 0x00, 0x45,
        0x00, 0x01, 0x38, 0xfd, 0x55, 0x00, 0x00, 0x40, 0x11, 0x78, 0x44, 0x4f, 0x8a, 0x32, 0x02,
        0x4f, 0x8a, 0x33, 0x05, 0x00, 0x43, 0x00, 0x44, 0x01, 0x24, 0x00, 0x00, 0x02, 0x01, 0x06,
        0x00, 0x51, 0x35, 0xb6, 0xa1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4f, 0x8a,
        0x33, 0x05, 0x00, 0x00, 0x00, 0x00, 0x0a, 0xb5, 0x04, 0x01, 0x00, 0x23, 0xc1, 0xde, 0xd0,
        0x0d, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        0xff, 0xfe, 0x00, 0x03, 0x04, 0x4f, 0x8a, 0x32, 0x01, 0x06, 0x08, 0x4f, 0x8a, 0x00, 0xb4,
        0x55, 0x08, 0x1f, 0xd1, 0x1c, 0x04, 0x4f, 0x8a, 0x33, 0xff, 0x33, 0x04, 0x00, 0x00, 0x54,
        0x49, 0x35, 0x01, 0x05, 0x36, 0x04, 0x0a, 0xb5, 0x04, 0x01, 0xff
    };

    u8_t relay_ack2[] = {
        0x00, 0x23, 0xc1, 0xde, 0xd0, 0x0d, 0x00, 0x22, 0x93, 0x5a, 0xf7, 0x60, 0x08, 0x00, 0x45,
        0x00, 0x01, 0x38, 0xfa, 0x18, 0x00, 0x00, 0x40, 0x11, 0x7b, 0x81, 0x4f, 0x8a, 0x32, 0x02,
        0x4f, 0x8a, 0x33, 0x05, 0x00, 0x43, 0x00, 0x44, 0x01, 0x24, 0x00, 0x00, 0x02, 0x01, 0x06,
        0x00, 0x49, 0x8b, 0x6e, 0xab, 0x00, 0x00, 0x00, 0x00, 0x4f, 0x8a, 0x33, 0x05, 0x4f, 0x8a,
        0x33, 0x05, 0x00, 0x00, 0x00, 0x00, 0x0a, 0xb5, 0x04, 0x01, 0x00, 0x23, 0xc1, 0xde, 0xd0,
        0x0d, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        ...
        0xff, 0xfe, 0x00, 0x03, 0x04, 0x4f, 0x8a, 0x32, 0x01, 0x06, 0x08, 0x4f, 0x8a, 0x00, 0xb4,
        0x55, 0x08, 0x1f, 0xd1, 0x1c, 0x04, 0x4f, 0x8a, 0x33, 0xff, 0x33, 0x04, 0x00, 0x00, 0x54,
        0x60, 0x35, 0x01, 0x05, 0x36, 0x04, 0x0a, 0xb5, 0x04, 0x01, 0xff
    };
    ...
    for (i = 0; i < 108000 - 25; i++) { tick_lwip(); }

    fail_unless(netif_is_up(&net_test));
    fail_unless(txpacket == (3 + DHCP_TEST_NUM_ARP_FRAMES), "TX %d packets, expected %d", txpacket,
                (3 + DHCP_TEST_NUM_ARP_FRAMES));

    /* We need to send arp response here.. */

    send_pkt(&net_test, arp_resp, sizeof(arp_resp));

    fail_unless(txpacket == (4 + DHCP_TEST_NUM_ARP_FRAMES), "TX %d packets, expected %d", txpacket,
                (4 + DHCP_TEST_NUM_ARP_FRAMES));
    fail_unless(netif_is_up(&net_test));
    ...
    for (i = 0; i < 100000; i++) { tick_lwip(); }

    fail_unless(txpacket == (4 + DHCP_TEST_NUM_ARP_FRAMES), "TX %d packets, expected %d", txpacket,
                (5 + DHCP_TEST_NUM_ARP_FRAMES));
...
START_TEST(test_dhcp_nak_no_endmarker) {
    ...
    u8_t dhcp_nack_no_endmarker[] = {
        ,
        ...
        ,
