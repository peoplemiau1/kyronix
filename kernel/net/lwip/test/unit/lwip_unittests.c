...
unsigned int lwip_port_rand(void) { return (unsigned int) rand(); }

Suite *create_suite(const char *name, testfunc *tests, size_t num_tests, SFun setup,
                    SFun teardown) {
    size_t i;
...
void lwip_check_ensure_no_alloc(unsigned int skip) {
    ...
    if (!(skip & SKIP_HEAP)) {
        fail_unless(lwip_stats.mem.used == 0, "mem heap still has %d bytes allocated",
                    lwip_stats.mem.used);
    }
    for (i = 0, mask = 1; i < MEMP_MAX; i++, mask <<= 1) {
        if (!(skip & mask)) {
            ...
            fail_unless(lwip_stats.memp[i]->used == 0,
                        "memp pool '%s' still has %d entries allocated", lwip_stats.memp[i]->name,
                        lwip_stats.memp[i]->used);
#else
            fail_unless(lwip_stats.memp[i]->used == 0,
                        "memp pool %d still has %d entries allocated", i, lwip_stats.memp[i]->used);
#endif
        }
    }
}
...
#endif
{
    ...
    SRunner *sr;
    size_t i;
    suite_getter_fn *suites[] = { ip4_suite,
                                  ip6_suite,
                                  udp_suite,
                                  tcp_suite,
                                  tcp_oos_suite,
    ...
    for (i = 1; i < num; i++) { srunner_add_suite(sr, ((suite_getter_fn *) suites[i])()); }
