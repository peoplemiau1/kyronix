...
static struct pbuf *tcp_create_segment_wnd(ip_addr_t *src_ip, ip_addr_t *dst_ip, u16_t src_port,
                                           u16_t dst_port, void *data, size_t data_len, u32_t seqno,
                                           u32_t ackno, u8_t headerflags, u16_t wnd) {
    struct pbuf *p, *q;
    struct ip_hdr *iphdr;
    ...
    for (q = p; q != NULL; q = q->next) { memset(q->payload, 0, q->len); }
...
/** Create a TCP segment usable for passing to tcp_input */
struct pbuf *tcp_create_segment(ip_addr_t *src_ip, ip_addr_t *dst_ip, u16_t src_port,
                                u16_t dst_port, void *data, size_t data_len, u32_t seqno,
                                u32_t ackno, u8_t headerflags) {
    return tcp_create_segment_wnd(src_ip, dst_ip, src_port, dst_port, data, data_len, seqno, ackno,
                                  headerflags, TCP_WND);
}
...
struct pbuf *tcp_create_rx_segment(struct tcp_pcb *pcb, void *data, size_t data_len,
    ...
    return tcp_create_segment(&pcb->remote_ip, &pcb->local_ip, pcb->remote_port, pcb->local_port,
                              data, data_len, pcb->rcv_nxt + seqno_offset,
...
struct pbuf *tcp_create_rx_segment_wnd(struct tcp_pcb *pcb, void *data, size_t data_len,
                                       u32_t seqno_offset, u32_t ackno_offset, u8_t headerflags,
                                       u16_t wnd) {
    return tcp_create_segment_wnd(&pcb->remote_ip, &pcb->local_ip, pcb->remote_port,
                                  pcb->local_port, data, data_len, pcb->rcv_nxt + seqno_offset,
