...
static int tcp_oos_count(struct tcp_pcb *pcb) {
    ...
    (PBUF_POOL_BUFSIZE >= (TCP_MSS + PBUF_LINK_ENCAPSULATION_HLEN + PBUF_LINK_HLEN +               \
...
static u32_t tcp_oos_seg_seqno(struct tcp_pcb *pcb, int seg_index) {
    ...
    while (seg != NULL) {
        if (num == seg_index) { return seg->tcphdr->seqno; }
        num++;
        seg = seg->next;
    }
    fail();
...
static int tcp_oos_seg_tcplen(struct tcp_pcb *pcb, int seg_index) {
    ...
    /* then check the actual segment */
    while (seg != NULL) {
        if (num == seg_index) { return TCP_TCPLEN(seg); }
        num++;
        seg = seg->next;
    }
    fail();
...
START_TEST(test_tcp_recv_ooseq_FIN_OOSEQ) {
    ...
    EXPECT_RET(pcb != NULL);
    tcp_set_state(pcb, ESTABLISHED, &test_local_ip, &test_remote_ip, TEST_LOCAL_PORT,
                  TEST_REMOTE_PORT);
    ...
    EXPECT(p_2_14 != NULL);
    EXPECT(p_fin != NULL);
    if ((pinseq != NULL) && (p_8_9 != NULL) && (p_4_8 != NULL) && (p_4_10 != NULL) &&
        (p_2_14 != NULL) && (p_fin != NULL)) {
...
START_TEST(test_tcp_recv_ooseq_FIN_INSEQ) {
    ...
    if ((pinseq != NULL) && (p_1_2 != NULL) && (p_4_8 != NULL) && (p_3_11 != NULL) &&
        (p_2_12 != NULL) && (p_15_1 != NULL) && (p_15_1a != NULL) && (pinseqFIN != NULL)) {
        /* pass the segment to tcp_input */
...
START_TEST(test_tcp_recv_ooseq_overrun_rxwin) {
    ...
    for (i = 0; i < (int) sizeof(data_full_wnd); i++) { data_full_wnd[i] = (char) i; }
    ...
    for (i = TCP_MSS, k = 0; i < TCP_WND; i += TCP_MSS, k++) {
        ...
        struct pbuf *p = tcp_create_rx_segment(pcb, &data_full_wnd[TCP_MSS * (k + 1)], TCP_MSS,
                                               TCP_MSS * (k + 1), 0, TCP_ACK);
        EXPECT_RET(p != NULL);
        /* pass the segment to tcp_input */
        ...
        if (datalen != expected_datalen) { EXPECT_OOSEQ(datalen == expected_datalen); }
    }

    /* pass in one more segment, clearly overrunning the rxwin */
    p_ovr = tcp_create_rx_segment(pcb, &data_full_wnd[TCP_MSS * (k + 1)], TCP_MSS,
                                  TCP_MSS * (k + 1), 0, TCP_ACK);
...
START_TEST(test_tcp_recv_ooseq_max_bytes) {
    ...
    for (i = 0; i < sizeof(data_full_wnd); i++) { data_full_wnd[i] = (char) i; }
    ...
    tcp_abort(pcb);
    EXPECT(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 0);
#endif /* TCP_OOSEQ_MAX_BYTES && (TCP_OOSEQ_MAX_BYTES < (TCP_WND + 1)) && (PBUF_POOL_BUFSIZE >=    \
          (TCP_MSS + PBUF_LINK_ENCAPSULATION_HLEN + PBUF_LINK_HLEN + PBUF_IP_HLEN +                \
          PBUF_TRANSPORT_HLEN)) */
    LWIP_UNUSED_ARG(_i);
}
END_TEST
...
START_TEST(test_tcp_recv_ooseq_max_pbufs) {
    ...
    tcp_abort(pcb);
    EXPECT(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 0);
#endif /* TCP_OOSEQ_MAX_PBUFS && (TCP_OOSEQ_MAX_BYTES < (TCP_WND + 1)) && (PBUF_POOL_BUFSIZE >=    \
          (TCP_MSS + PBUF_LINK_ENCAPSULATION_HLEN + PBUF_LINK_HLEN + PBUF_IP_HLEN +                \
          PBUF_TRANSPORT_HLEN)) */
    LWIP_UNUSED_ARG(_i);
}
...
static void check_rx_counters(struct tcp_pcb *pcb, struct test_tcp_counters *counters,
                              u32_t exp_close_calls, u32_t exp_rx_calls, u32_t exp_rx_bytes,
                              u32_t exp_err_calls, int exp_oos_count, int exp_oos_len) {
    int oos_len;
...
static void test_tcp_recv_ooseq_double_FINs(int delay_packet) {
    ...
    struct netif netif;
    u32_t exp_rx_calls = 0, exp_rx_bytes = 0, exp_close_calls = 0, exp_oos_pbufs = 0,
          exp_oos_tcplen = 0;
    ...
    k = 1;
    p_data_after_fin =
        tcp_create_rx_segment(pcb, &data_full_wnd[TCP_MSS + 1], k, TCP_MSS + 1, 0, TCP_ACK);
    ...
    /* check if counters are as expected */
    check_rx_counters(pcb, &counters, exp_close_calls, exp_rx_calls, exp_rx_bytes, 0, exp_oos_pbufs,
                      exp_oos_tcplen);

    if (delay_packet & 2) {
        /* drop FIN */
        if (first_dropped > 2) { first_dropped = 2; }
    } else {
        /* send FIN */
    ...
    if (delay_packet & 4) {
        /* drop data-after-FIN */
        if (first_dropped > 3) { first_dropped = 3; }
    } else {
        /* send data-after-FIN */
    ...
    if (delay_packet & 8) {
        /* drop 2nd-FIN */
        if (first_dropped > 4) { first_dropped = 4; }
    } else {
        /* send 2nd-FIN */
...
Suite *tcp_oos_suite(void) {
    testfunc tests[] = { TESTFUNC(test_tcp_recv_ooseq_FIN_OOSEQ),
    ...
    return create_suite("TCP_OOS", tests, sizeof(tests) / sizeof(testfunc), tcp_oos_setup,
                        tcp_oos_teardown);
}
