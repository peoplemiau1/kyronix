...
static void test_tcp_tmr(void) {
    tcp_fasttmr();
    if (++test_tcp_timer & 1) { tcp_slowtmr(); }
}
...
static struct tcp_pcb_listen *create_listening_pcb(u16_t local_port,
...
static void test_rst_generation_with_incoming_packet(struct pbuf *p, struct netif *netif,
                                                     struct test_tcp_txcounters *tx_counters) {
    u16_t tcp_flags;
    EXPECT_RET(p != NULL);
...
START_TEST(test_tcp_new_max_num) {
    ...
    tcp_set_state(pcb[0], TIME_WAIT, &test_local_ip, &test_remote_ip, TEST_LOCAL_PORT,
                  TEST_REMOTE_PORT);
    pcb[MEMP_NUM_TCP_PCB] = tcp_new();
    fail_unless(pcb[MEMP_NUM_TCP_PCB] != NULL);
    fail_unless(MEMP_STATS_GET(used, MEMP_TCP_PCB) == MEMP_NUM_TCP_PCB);

    for (i = 1; i <= MEMP_NUM_TCP_PCB; i++) { tcp_abort(pcb[i]); }
    fail_unless(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 0);
}
END_TEST
...
START_TEST(test_tcp_new_max_num_remove_TIME_WAIT) {
    ...
    pcb = tcp_new();
    EXPECT_RET(pcb != NULL);
    tcp_set_state(pcb, TIME_WAIT, &test_local_ip, &test_remote_ip, TEST_LOCAL_PORT,
                  TEST_REMOTE_PORT);
    ...
    for (i = 0; i <= MEMP_NUM_TCP_PCB - 1; i++) { tcp_abort(pcb_list[i]); }
...
START_TEST(test_tcp_connect_active_open) {
    ...
    p = tcp_create_segment(&pcb->remote_ip, &pcb->local_ip, test_port, pcb->local_port, NULL, 0,
                           12345, seqno + 1, TCP_SYN | TCP_ACK);
    EXPECT_RET(p != NULL);
...
START_TEST(test_tcp_active_close) {
    ...
    EXPECT_RET(pcb->state == TIME_WAIT);
    EXPECT_RET(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 1);
    for (i = 0; i < 2 * TCP_MSL / TCP_TMR_INTERVAL + 1; i++) { test_tcp_tmr(); }
    EXPECT_RET(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 0);
}
END_TEST
...
START_TEST(test_tcp_imultaneous_close) {
    ...
    EXPECT(p != NULL);
    if (p != NULL) { test_tcp_input(p, &test_netif); }
...
START_TEST(test_tcp_gen_rst_in_CLOSED) {
    ...
    /* create a segment */
    p = tcp_create_segment(&src_addr, &dst_addr, TEST_REMOTE_PORT, TEST_LOCAL_PORT, NULL, 0, 12345,
                           54321, TCP_ACK);
    EXPECT(p != NULL);
...
START_TEST(test_tcp_gen_rst_in_LISTEN) {
    ...
    /* create a segment */
    p = tcp_create_segment(&src_addr, &lpcb->local_ip, TEST_REMOTE_PORT, lpcb->local_port, NULL, 0,
                           12345, 54321, TCP_ACK);
    EXPECT(p != NULL);
    test_rst_generation_with_incoming_packet(p, &test_netif, &test_txcounters);
    EXPECT(test_txcounters.num_tx_calls == 1);
...
START_TEST(test_tcp_process_rst_seqno) {
    ...
    p = tcp_create_segment(&pcb->remote_ip, &pcb->local_ip, TEST_REMOTE_PORT, pcb->local_port, NULL,
                           0, 12345, pcb->snd_nxt - 10, TCP_RST);
    EXPECT(p != NULL);
    test_tcp_input(p, &test_netif);
    ...
    /* a RST segment with incorrect seqno will not be accepted */
    p = tcp_create_segment(&pcb->remote_ip, &pcb->local_ip, pcb->remote_port, pcb->local_port, NULL,
                           0, pcb->rcv_nxt - 10, 54321, TCP_RST);
    EXPECT(p != NULL);
    test_tcp_input(p, &test_netif);
    EXPECT(counters.err_calls == 0);
...
START_TEST(test_tcp_gen_rst_in_SYN_SENT_ackseq) {
    ...
    p = tcp_create_segment(&pcb->remote_ip, &pcb->local_ip, pcb->remote_port, pcb->local_port, NULL,
                           0, 12345, pcb->lastack - 10, TCP_SYN | TCP_ACK);
    EXPECT(p != NULL);
    test_rst_generation_with_incoming_packet(p, &test_netif, &test_txcounters);
...
START_TEST(test_tcp_gen_rst_in_SYN_RCVD) {
    ...
    EXPECT(test_txcounters.num_tx_calls == 1);
    EXPECT(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 1);
    if (MEMP_STATS_GET(used, MEMP_TCP_PCB) == 1) { ack_seqno = tcp_active_pcbs[0].lastack; }

    /* create a ACK segment with incorrect seqno */
    p = tcp_create_segment(&src_addr, &lpcb->local_ip, TEST_REMOTE_PORT, lpcb->local_port, NULL, 0,
                           1001, ack_seqno + 1111, TCP_ACK);
    EXPECT(p != NULL);
    test_rst_generation_with_incoming_packet(p, &test_netif, &test_txcounters);
    EXPECT(test_txcounters.num_tx_calls == 1);
...
Suite *tcp_state_suite(void) {
    ...
    return create_suite("TCP_STATE", tests, sizeof(tests) / sizeof(testfunc), tcp_state_setup,
                        tcp_state_teardown);
}
