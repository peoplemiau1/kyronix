...
START_TEST(test_tcp_listen_passive_open) {
    ...
    ip_addr_set_ip4_u32_val(src_addr,
                            lwip_htonl(lwip_ntohl(ip_addr_get_ip4_u32(&lpcb->local_ip)) + 1));

    /* check correct syn packet */
    p = tcp_create_segment(&src_addr, &lpcb->local_ip, 12345, lpcb->local_port, NULL, 0, 12345,
                           54321, TCP_SYN);
    EXPECT(p != NULL);
    if (p != NULL) {
...
START_TEST(test_tcp_recv_inseq_trim) {
    ...
    if (p != NULL) {
        EXPECT(p->next != NULL);
        if (p->next != NULL) { EXPECT(p->next->next != NULL); }
    }
...
START_TEST(test_tcp_fast_retx_recover) {
    ...
    do { err = tcp_write(pcb, data6, TCP_MSS, TCP_WRITE_FLAG_COPY); } while (err == ERR_OK);
    EXPECT_RET(err != ERR_OK);
...
START_TEST(test_tcp_fast_rexmit_wraparound) {
    ...
    for (i = 0; i < sizeof(tx_data); i++) { tx_data[i] = (u8_t) i; }
...
START_TEST(test_tcp_retx_add_to_sent) {
    ...
    EXPECT_RET(txcounters.num_tx_calls == 1);
    EXPECT_RET(txcounters.num_tx_bytes ==
               sizeof(data1a) + sizeof(data1b) + sizeof(struct tcp_hdr) + sizeof(struct ip_hdr));
    memset(&txcounters, 0, sizeof(txcounters));
    /* "recv" ACK for data1 */
    p = tcp_create_rx_segment(pcb, NULL, 0, 0, 4, TCP_ACK);
    EXPECT_RET(p != NULL);
    ...
    EXPECT_RET(txcounters.num_tx_calls == 1);
    EXPECT_RET(txcounters.num_tx_bytes ==
               sizeof(data2a) + sizeof(data2b) + sizeof(struct tcp_hdr) + sizeof(struct ip_hdr));
    memset(&txcounters, 0, sizeof(txcounters));
    /* send data3 */
    err = tcp_write(pcb, data3, sizeof(data3), TCP_WRITE_FLAG_COPY);
    EXPECT_RET(err == ERR_OK);
    ...
    for (i = 0; i < 20; i++) {
        test_tcp_tmr();
        if (txcounters.num_tx_calls != 0) { break; }
    }
    /* data3 sent */
    EXPECT_RET(txcounters.num_tx_calls == 1);
    EXPECT_RET(txcounters.num_tx_bytes ==
    ...
    EXPECT_RET(txcounters.num_tx_calls == 1);
    EXPECT_RET(txcounters.num_tx_bytes ==
               sizeof(data3) + sizeof(data4) + sizeof(struct tcp_hdr) + sizeof(struct ip_hdr));
    memset(&txcounters, 0, sizeof(txcounters));
    /* make sure the pcb is freed */
    EXPECT_RET(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 1);
    tcp_abort(pcb);
...
START_TEST(test_tcp_rto_tracking) {
    ...
    while (!(pcb->flags & TF_RTO)) { test_tcp_tmr(); }
...
static void test_tcp_rto_timeout_impl(int link_down) {
    ...
    for (i = 0; i < TCP_MSS; i++) { tx_data[i] = (u8_t) i; }
    ...
    /* Force us into retransmisson timeout */
    for (i = 0; !(pcb->flags & TF_RTO) && i < max_wait_ctr; i++) { test_tcp_tmr(); }
    ...
    if (link_down) { netif_set_link_down(&netif); }

    /* keep running the timer till we hit our maximum RTO */
    for (i = 0; counters.last_err == ERR_OK && i < max_wait_ctr; i++) { test_tcp_tmr(); }
    EXPECT(i < max_wait_ctr);
    ...
    for (cur = tcp_active_pcbs; cur != NULL; cur = cur->next) { EXPECT(cur != pcb); }
    EXPECT_RET(MEMP_STATS_GET(used, MEMP_TCP_PCB) == 0);
}
...
static void test_tcp_rto_timeout_syn_sent_impl(int link_down) {
    ...
    size_t i;
    const size_t max_wait_ctr = 1024 * 1024;
    const u16_t tcp_syn_opts_len = LWIP_TCP_OPT_LENGTH(TF_SEG_OPTS_MSS | TF_SEG_OPTS_WND_SCALE |
...
static void test_tcp_zwp_timeout_impl(int link_down) {
    ...
    for (i = 0; i < 2 * TCP_MSS; i++) { tx_data[i] = (u8_t) i; }
    ...
    while (pcb->persist_probe == 0) { test_tcp_tmr(); }
    EXPECT(txcounters.num_tx_calls == 1);
    EXPECT(txcounters.num_tx_bytes == (1 + 40U));
    memset(&txcounters, 0, sizeof(txcounters));
    ...
    while (counters.last_err == ERR_OK) { test_tcp_tmr(); }
...
START_TEST(test_tcp_persist_split) {
    ...
    for (i = 0; i < 4 * TCP_MSS; i++) { tx_data[i] = (u8_t) i; }
    ...
    err =
        tcp_write(pcb, &tx_data[0], (3 * TCP_MSS) + (TCP_MSS - (TCP_MSS / 4)), TCP_WRITE_FLAG_COPY);
    EXPECT(err == ERR_OK);
    err = tcp_output(pcb);
    EXPECT(err == ERR_OK);
