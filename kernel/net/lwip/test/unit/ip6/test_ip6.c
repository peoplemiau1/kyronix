...
static void ip6_test_handle_timers(int count) {
    int i;
    for (i = 0; i < count; i++) { nd6_tmr(); }
}

/* Helper functions */
static void create_ip6_input_fragment(u32_t ip_id, u16_t start, u16_t len, int last,
                                      u8_t next_hdr) {
    struct pbuf *p;
    ...
    p = pbuf_alloc(PBUF_RAW, len + sizeof(struct ip6_frag_hdr) + sizeof(struct ip6_hdr), PBUF_RAM);
    fail_unless(p != NULL);
    if (p != NULL) {
        err_t err;
...
START_TEST(test_ip6_lladdr) {
    ...
    const u32_t expected_ip6_addr_1[4] = { PP_HTONL(0xfe800000), 0, PP_HTONL(0xb2a1a2ff),
                                           PP_HTONL(0xfea3a4a5) };
    const u32_t expected_ip6_addr_2[4] = { PP_HTONL(0xfe800000), 0, PP_HTONL(0x0000b0a1),
                                           PP_HTONL(0xa2a3a4a5) };
    LWIP_UNUSED_ARG(_i);
...
START_TEST(test_ip6_dest_unreachable_chained_pbuf) {
    ...
    u8_t udp_hdr[] = {
        0x60, 0x00, 0x27, 0x03, 0x00, 0x2d, 0x11, 0x40, 0x20, 0x01, 0x0d, 0xb8,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04,
        0x20, 0x01, 0x0d, 0xb8, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x01, 0x01, 0xff, 0x03, 0xff, 0x00, 0x2d, 0x00, 0x00,
    };
    ...
    icmpptr += ICMP6_HLEN;
    fail_unless(memcmp(icmpptr, udp_hdr, sizeof(udp_hdr)) == 0,
                "mismatch in copied ip6/udp header");
    icmpptr += sizeof(udp_hdr);
    fail_unless(memcmp(icmpptr, udp_payload, sizeof(udp_payload)) == 0,
...
START_TEST(test_ip6_frag_pbuf_len_assert) {
    ...
    err = ip6_output_if_src(hdr, ip_2_ip6(&my_addr), ip_2_ip6(&peer_addr), 15, 0, IP_PROTO_UDP,
                            &test_netif6);
    fail_unless(err == ERR_OK);
...
START_TEST(test_ip6_frag) {
    ...
    err = ip6_output_if_src(data, ip_2_ip6(&my_addr), ip_2_ip6(&peer_addr), 15, 0, IP_PROTO_UDP,
                            &test_netif6);
    fail_unless(err == ERR_OK);
...
static void test_ip6_reass_helper(u32_t ip_id, const u16_t *segments, size_t num_segs,
                                  u16_t seglen) {
    ip_addr_t my_addr = IPADDR6_INIT_HOST(0x20010db8, 0x0, 0x0, 0x1);
    size_t i;
...
Suite *ip6_suite(void) {
    testfunc tests[] = { TESTFUNC(test_ip6_ll_addr),
...
START_TEST(test_ip6_dummy) { LWIP_UNUSED_ARG(_i); }
END_TEST
