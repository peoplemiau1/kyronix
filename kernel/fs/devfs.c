...
static void ptym_close(vfs_node_t* n)
{
    pty_inst_t* p = pty_of(n);
    if (!p) return;
    if (p->s2m) { p->s2m->write_refs = 0; pipe_free(p->s2m); p->s2m = NULL; }
    if (p->m2s) { pipe_free(p->m2s); p->m2s = NULL; }
    if (p->slave) {
        char path[32];
...
static int64_t ptys_write(vfs_node_t* n, const char* buf, uint64_t len)
{
    ...
    int64_t done = 0;
    for (uint64_t i = 0; i < len; i++) {
        if (buf[i] == '\n') { static const char cr = '\r'; pipe_write(p->s2m, &cr, 1); }
