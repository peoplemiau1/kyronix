...
static uint32_t ring_avail(const rx_ring_t* r)
{
    return (r->head - r->tail) & (RX_BUF_SZ - 1);
}
...
static err_t on_accept(void* arg, struct tcp_pcb* new_pcb, err_t err)
{
    ...
    if (!child) { tcp_abort(new_pcb); return ERR_MEM; }
...
static void on_udp_recv(void* arg, struct udp_pcb* pcb,
                        struct pbuf* p, const ip4_addr_t* addr, u16_t port)
{
    (void)pcb;
    net_conn_t* c = (net_conn_t*)arg;
    if (!c || !p) { if (p) pbuf_free(p); return; }

    int next = (c->udq_head + 1) & (UDP_RX_SLOTS - 1);
    if (next == c->udq_tail) { pbuf_free(p); return; } /* queue full, drop */
...
static uint8_t on_raw_recv(void* arg, struct raw_pcb* pcb,
...
int fd_inet_socket(int type, int flags)
{
    ...
    if (sock_type == SOCK_STREAM) {
        c->pcb = tcp_new();
        if (!c->pcb) { kfree(c); return -(int)ENOMEM; }
        tcp_arg(c->pcb, c);
    } else if (sock_type == SOCK_DGRAM) {
        c->upcb = udp_new();
        if (!c->upcb) { kfree(c); return -(int)ENOMEM; }
        udp_recv(c->upcb, on_udp_recv, c);
    } else {
        /* SOCK_RAW - proto is the IP protocol number */
        c->rpcb = raw_new((uint8_t)(flags & 0xFF));
        if (!c->rpcb) { kfree(c); return -(int)ENOMEM; }
        raw_recv(c->rpcb, on_raw_recv, c);
    }

    int fd = vfs_fd_alloc_from(0);
    if (fd < 0) {
        if (sock_type == SOCK_STREAM)      tcp_abort(c->pcb);
        else if (sock_type == SOCK_DGRAM)  udp_remove(c->upcb);
        else                               raw_remove(c->rpcb);
        kfree(c);
        return -(int)EMFILE;
    }
...
void inet_conn_close(net_conn_t* c)
{
    if (!c) return;
    if (c->type == SOCK_DGRAM) {
        if (c->upcb) { udp_remove(c->upcb); c->upcb = NULL; }
    } else if (c->type == SOCK_RAW) {
        if (c->rpcb) { raw_remove(c->rpcb); c->rpcb = NULL; }
    } else {
        if (c->pcb) {
            tcp_arg(c->pcb, NULL);
            tcp_recv(c->pcb, NULL);
...
int64_t inet_accept(net_conn_t* c, struct sockaddr_in* addr_out, int flags)
{
    ...
    int fd = vfs_fd_alloc_from(0);
    if (fd < 0) { inet_conn_close(child); return -(int64_t)EMFILE; }

    vfs_file_t* f = vfs_file_alloc();
    if (!f) { inet_conn_close(child); return -(int64_t)ENOMEM; }
