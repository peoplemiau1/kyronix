...
typedef struct { uint32_t id; uint32_t len; } __attribute__((packed)) vq_elem_t;
    ...
    while (g_rxq.last_used != g_rxq.used->idx) {
        ...
        if (tot > sizeof(vnet_hdr_t))
            net_receive(buf + sizeof(vnet_hdr_t),
                        (uint16_t)(tot - sizeof(vnet_hdr_t)));
...
bool virtnet_send(const uint8_t* data, uint16_t len)
{
    ...
    uint64_t flags = irq_save();
    while (g_txq.last_used != g_txq.used->idx)
        g_txq.last_used++;

    uint16_t outstanding = (uint16_t)(g_txq.next_free - g_txq.last_used);
    if (outstanding >= g_tx_nslots) { irq_restore(flags); return false; }
...
void virtnet_init(void)
{
    ...
    if (!dev) { log_warn("virtio-net: no PCI device"); return; }
    ...
    if (!(bar0 & 1u)) { log_warn("virtio-net: BAR0 not I/O"); return; }
    g_iobase = (uint16_t)(bar0 & ~3u);

    log_info("virtio-net: PCI %02x:%02x.%x iobase=0x%04x irq=%u",
             dev->bus, dev->dev, dev->fn, g_iobase, dev->irq_line);
    ...
    io_w8(REG_STATUS, S_ACK | S_DRIVER | S_FROK);
        ...
        //read mac
    for (int i = 0; i < 6; i++)
        g_mac[i] = io_r8((uint8_t)(REG_MAC + i));
    log_info("virtio-net: MAC %02x:%02x:%02x:%02x:%02x:%02x",
