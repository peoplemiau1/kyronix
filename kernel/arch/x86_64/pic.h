...
static inline void pic_remap(uint8_t off1, uint8_t off2) {
    ...
    outb(PIC1_DATA, off1); io_wait();   /* ICW2: master vector base */
    outb(PIC2_DATA, off2); io_wait();   /* ICW2: slave vector base */
    outb(PIC1_DATA, 4);    io_wait();   /* ICW3: slave on master IRQ2 */
    outb(PIC2_DATA, 2);    io_wait();   /* ICW3: slave cascade identity */
