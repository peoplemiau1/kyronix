...
int main(int argc, char **argv)
{
    ...
    bool flag_L = false;
    int first = 1;
    if (first < argc && strcmp(argv[first], "-L") == 0) { flag_L = true; first++; }
    if (first == argc) kx_die("usage: stat [-L] FILE...");
    int rc = 0;
    for (int i = first; i < argc; i++) {
        struct stat st;
        int r = flag_L ? stat(argv[i], &st) : lstat(argv[i], &st);
        if (r < 0) { kx_warn(argv[i]); rc = 1; continue; }
        char m[12]; mode_string(st.st_mode, m);
